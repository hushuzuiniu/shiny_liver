...
for (i in 1:length(unique_name)){
  results[[i]] <- read.csv(paste0(unique_name[i],'.results.csv'),header=TRUE,row.names = 1)
...
unique_name<- c('Histological diagnosis: yes V no',
                'Kleiner brunt steatosis: score 0 V score 3',
                'Modified ishak:score 0 V score 6',
                'Nafld ballooning:score 0 V score 2',
                'Nafld inflammation:score 0 V score 3',
                'Nash fibrosis:score 0 V score 4',
                'Saf balloon:score 0 V score 2',
                'Saf inflammation:score 0 V score 2',
                'Saf steatosis:score 0 V score 3')
...
# add the control column for the sample data
controls<-rep("disease",length(sample_detail[,1]))
controls[which(row.names(sample_detail) %in% controls_ids[,1])]<-"control"
sample_detail[['control']]<-controls
      ...
      tabItem(tabName = "PCA",
              ...
              fluidRow(
                ...
                box(
                  title = "Point Size",
                  width = 4,
                  sliderInput("slider1", label=NULL, 0.5, 10, 1)
                )),
              fluidRow(
                box(
                  title = "Legend Group",
...
server <- function(input, output,session) {
  ...
  tableInput2<-reactive({
    if(input$sel10 == "GO_Biological_Processes"){
      pathway_table <- GOBPs[[input$sel.group]][which(GOBPs[[input$sel.group]]$FDR<0.5),]
      
    }else if(input$sel10 == "GO_Cellular_Components"){
      pathway_table <- GOCCs[[input$sel.group]][which(GOCCs[[input$sel.group]]$FDR<0.5),]
      
    }else if(input$sel10 == "GO_Molecular_Functions"){
      pathway_table <- GOMFs[[input$sel.group]][which(GOMFs[[input$sel.group]]$FDR<0.5),]
      
    }else if(input$sel10 == "KEGG_Pathways"){
      pathway_table <- KEGGs[[input$sel.group]][which(KEGGs[[input$sel.group]]$FDR<0.5),]
      
    }else{
      pathway_table <- Reactomes[[input$sel.group]][which(Reactomes[[input$sel.group]]$FDR<0.5),]
